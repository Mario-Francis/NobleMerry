...
class PaymentsService{
    ...
    // add investor reg payment by transfer
    public function update_investor_payment_by_transfer(int $c_user_id, int $payment_id){
        // get reg fee
        ...
        $this->investor_payment_model->update($payment_id, $update_data);

        $pay_log_data= [
            'investor_payment_id'=>$payment['id'],
            'amount'=>$payment['amount'],
            'paid_by_id'=>$c_user_id,
            'paid_date'=>$now,
            'status'=>PAY_LOG_PENDING,
    ...
    public function confirm_investor_payment_by_transfer(int $user_id, int $payment_id, $action = PAY_REVIEW_ACTION_CONFIRM){
        // get reg fee
        $payment = $this->investor_payment_model->find($payment_id);
        ...
        $this->investor_payment_model->update($payment['id'], $update_data);
        $pay_log = $this->payment_log_model->where('investor_payment_id', $payment['id'])->first();
        $pay_log_data= [
            'investor_payment_id'=>$payment['id'],
            'amount'=>$payment['amount'],
